...
async function orcaOpenSingleSidedPosition(agent, whirlpoolAddress, distanceFromCurrentPriceBps, widthBps, inputTokenMint, inputAmount) {
    try {
        ...
        if (txBuilderTickArrays !== null) {
            ...
            const signers = txPayloadTickArrays.signers;
            const tickArrayTxId = await (0, send_tx_1.sendTx)(agent, instructions, signers);
            txIds += tickArrayTxId + ",";
        }
        ...
        const positionTxId = await (0, send_tx_1.sendTx)(agent, instructions, signers);
        txIds += positionTxId;
