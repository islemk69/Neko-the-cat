...
import { FIXED_MANIFEST_HEADER_SIZE, NIL, NO_EXPIRATION_LAST_VALID_SLOT, } from './constants';
import { claimedSeatBeet, createCreateMarketInstruction, PROGRAM_ID, restingOrderBeet, } from './manifest';
...
export class Market {
    ...
    static loadFromBuffer({ address, buffer, slot, }) {
        const marketData = Market.deserializeMarketBuffer(buffer, slot ?? NO_EXPIRATION_LAST_VALID_SLOT);
    ...
    getBalances(trader) {
        ...
        const quoteWithdrawableBalanceTokens = toNum(seat.quoteBalance) / 10 ** this.quoteDecimals();
        const baseWithdrawableBalanceTokens = toNum(seat.baseBalance) / 10 ** this.baseDecimals();
        return {
    ...
    static deserializeMarketBuffer(data, currentSlot) {
        ...
        const bids = bidsRootIndex != NIL
            ? deserializeRedBlackTree(data.subarray(FIXED_MANIFEST_HEADER_SIZE), bidsRootIndex, restingOrderBeet)
                ...
                .filter((bid) => {
                return (bid.lastValidSlot == NO_EXPIRATION_LAST_VALID_SLOT ||
                    Number(bid.lastValidSlot) > currentSlot);
            })
            : [];
        const asks = asksRootIndex != NIL
            ? deserializeRedBlackTree(data.subarray(FIXED_MANIFEST_HEADER_SIZE), asksRootIndex, restingOrderBeet)
                .map((restingOrderInternal) => {
                return {
                    trader: publicKeyBeet.deserialize(data.subarray(Number(restingOrderInternal.traderIndex) +
                        16 +
                ...
                .filter((ask) => {
                return (ask.lastValidSlot == NO_EXPIRATION_LAST_VALID_SLOT ||
                    Number(ask.lastValidSlot) > currentSlot);
            })
            : [];
        const claimedSeats = claimedSeatsRootIndex != NIL
            ? deserializeRedBlackTree(data.subarray(FIXED_MANIFEST_HEADER_SIZE), claimedSeatsRootIndex, claimedSeatBeet).map((claimedSeatInternal) => {
                return {
