...
import { createCreateWrapperInstruction, createPlaceOrderInstruction, createSettleFundsInstruction, OrderType, PROGRAM_ID, wrapperOpenOrderBeet as uiWrapperOpenOrderBeet, } from './ui_wrapper';
...
import { marketInfoBeet, } from './ui_wrapper/types';
/**
...
export class UiWrapper {
    ...
    static deserializeWrapperBuffer(data) {
        ...
        const parsedMarketInfos = marketInfos.map((marketInfoRaw) => {
            ...
            const rawOpenOrders = rootIndex != NIL
    ...
    placeOrderIx(market, accounts, args) {
        ...
        const traderTokenAccount = getAssociatedTokenAddressSync(mint, owner, true, traderTokenProgram);
    ...
    static async placeOrderCreateIfNotExistsIxs(connection, baseMint, baseDecimals, quoteMint, quoteDecimals, owner, payer, args, baseTokenProgram = TOKEN_PROGRAM_ID, quoteTokenProgram = TOKEN_PROGRAM_ID) {
        const ixs = [];
        const signers = [];
    ...
    static placeIx_(market, accounts, args) {
        ...
        const traderTokenProgram = isBid
            ? accounts.quoteTokenProgram
            : accounts.baseTokenProgram;
        const traderTokenAccount = getAssociatedTokenAddressSync(mint, accounts.owner, true, traderTokenProgram);
        const vault = getVaultAddress(market.address, mint);
        const clientOrderId = args.orderId ?? Date.now();
        const baseAtoms = Math.round(args.amount * 10 ** market.baseDecimals());
        let priceMantissa = args.price;
