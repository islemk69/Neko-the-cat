...
import { createAssociatedTokenAccountIdempotent, getAssociatedTokenAddress, mintTo, } from '@solana/spl-token';
...
export async function swap(connection, payerKeypair, marketAddress, amountAtoms, isBid, minOutAtoms = 0) {
    ...
    const swapIx = client.swapIx(payerKeypair.publicKey, {
        inAtoms: amountAtoms,
...
async function testSwapGlobal() {
    const connection = new Connection('http://127.0.0.1:8899', 'confirmed');
    const payerKeypair = Keypair.generate();
    const restingOrderTraderKeypair = Keypair.generate();
    ...
    const traderBaseTokenAccount = await createAssociatedTokenAccountIdempotent(connection, payerKeypair, market.baseMint(), payerKeypair.publicKey);
    // Initialize trader quote so they can receive.
    await createAssociatedTokenAccountIdempotent(connection, payerKeypair, market.quoteMint(), payerKeypair.publicKey);
    const amountBaseAtoms = 1_000_000_000;
    const mintSig = await mintTo(connection, payerKeypair, market.baseMint(), traderBaseTokenAccount, payerKeypair.publicKey, amountBaseAtoms);
    ...
    await placeOrder(connection, restingOrderTraderKeypair, marketAddress, 5, 5, true, OrderType.Global, 1, NO_EXPIRATION_LAST_VALID_SLOT);
    ...
    assert(market.getWithdrawableBalanceTokens(payerKeypair.publicKey, false) == 0, `Expected quote ${0} actual quote ${market.getWithdrawableBalanceTokens(payerKeypair.publicKey, false)}`);
    assert(market.getWithdrawableBalanceTokens(payerKeypair.publicKey, true) == 0, `Expected base ${0} actual base ${market.getWithdrawableBalanceTokens(payerKeypair.publicKey, true)}`);
    const swapperBaseBalance = (await connection.getTokenAccountBalance(await getAssociatedTokenAddress(market.baseMint(), payerKeypair.publicKey))).value.uiAmount;
    const swapperQuoteBalance = (await connection.getTokenAccountBalance(await getAssociatedTokenAddress(market.quoteMint(), payerKeypair.publicKey))).value.uiAmount;
    assert(swapperBaseBalance == 0, `Expected wallet base ${0} actual base ${swapperBaseBalance}`);
    // 5, received from matching the global order.
    assert(swapperQuoteBalance == 5, `Expected  quote ${5} actual quote ${swapperQuoteBalance}`);
}
describe('Swap test', () => {
    it('Swap', async () => {
        await testSwap();
