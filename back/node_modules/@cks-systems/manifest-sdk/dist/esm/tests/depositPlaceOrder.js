...
async function testDepositPlaceOrder() {
    ...
    await depositPlaceOrder(connection, payerKeypair, marketAddress, 4, 5, false, OrderType.Limit, 0);
    const traderTokenAccount = await createAssociatedTokenAccountIdempotent(connection, payerKeypair, market.quoteMint(), payerKeypair.publicKey);
    const quoteSize = 3;
    const quotePrice = 3;
    const quoteNotional = quotePrice * quoteSize;
    const amountAtoms = Math.ceil(quoteNotional * 10 ** market.quoteDecimals());
    const mintSig = await mintTo(connection, payerKeypair, market.quoteMint(), traderTokenAccount, payerKeypair.publicKey, amountAtoms);
    console.log('Minted quote tokens', mintSig);
    await depositPlaceOrder(connection, payerKeypair, marketAddress, quoteSize, quotePrice, true, OrderType.Limit, 1);
    await market.reload(connection);
    market.prettyPrint();
    // Asks are sorted worst to best.
    assert(market.asks().length == 1, 'place asks did not work');
    assert(Number(market.asks()[0].numBaseTokens) == 4, 'ask top of book wrong size');
    ...
    assert(market.getWithdrawableBalanceTokens(payerKeypair.publicKey, true) == 6, 'withdraw withdrawable balance check base');
    assert(market.getWithdrawableBalanceTokens(payerKeypair.publicKey, false) == 0, 'withdraw withdrawable balance check quote');
    assert(JSON.stringify(market.getBalances(payerKeypair.publicKey)) ==
        JSON.stringify({
...
export async function depositPlaceOrder(connection, payerKeypair, marketAddress, numBaseTokens, tokenPrice, isBid, orderType, clientOrderId, lastValidSlot = 0) {
    const client = await ManifestClient.getClientForMarket(connection, marketAddress, payerKeypair);
    const depositPlaceOrderIx = await client.placeOrderWithRequiredDepositIx(payerKeypair.publicKey, {
        numBaseTokens,
        tokenPrice,
        isBid,
    ...
    const signature = await sendAndConfirmTransaction(connection, new Transaction().add(...depositPlaceOrderIx), [payerKeypair]);
